...
<br />
For details look here: <a class="font-medium text-blue-600 hover:underline" href="https://github.com/tedsteen/nes-bundler">https://github.com/tedsteen/nes-bundler</a>
</p>
<style>
    #file-area * {pointer-events: none;}
</style>
<form method="POST" enctype="multipart/form-data" action="?/bundle" class="max-w-sm mx-auto">
    <div class="mb-5">
        <input name="recipient_email" type="email" value={form?.recipient_email ?? ''} id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="your@email.com" required />
    </div>
    <div class="flex items-start mb-5">
        <label id="file-area" on:drop={ drop } class:bg-gray-300={dragHover} on:dragleave={(ev) => { dragHover = false }} on:dragenter={(ev) => { dragHover = true }} for="dropzone-file" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-300">
    ...
    {/if}
    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Bundle!</button>
</form>
